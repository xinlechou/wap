...
function syn_deal($deal_id)
{
	...
	if($deal_info)
	{
		...
		$deal_info['support_amount'] = doubleval($GLOBALS['db']->getOne("select sum(price) from ".DB_PREFIX."deal_order where deal_id = ".$deal_info['id']." and order_status=3 "));
...
function send_pay_success($log_info){
	...
	foreach ($deal_s_user as $k=>$v){
		...
		$msg_data3['title']='项目成功发起者-'.$v['name'];
		$msg_data3['is_send']=0;
