            ...
            user.sendAjax(url,p,function(json){
                if(json.status){
                    $.showSuccess(json.info,function(){
                        location.href=APP_ROOT+"/index.php?ctl=olduser&act=login";//APP_ROOT+"/index.php?ctl=user&act=login";
                    });
                }else{
                    $.showErr(json.info);
                }
            ...
            user.sendAjax(url,p,function(json){
                ...
                if(json.status==1){
                    $.showSuccess("保存成功!",function(){
                        window.location.href = APP_ROOT+"/index.php?ctl=oldsettings";
                    });
                }
                if(json.status==0){
                    $.showSuccess(json.info);
