...
$(function(){
    var user = {
        ...
        codeLeftTimeFun: function () {
            ...
            oBtn.html(user.leftTime + "秒后可重新发送").addClass("disabled");
            ...
            if (user.leftTime > 0) {
                ...
                user.leftTime = 60;
                oBtn.removeAttr("disabled").removeClass("disabled").html("获取验证码");//启用按钮
            }
        },resultTmp:{},
        //检查验证码
        checkRegisterVerifyCoder: function () {
            ...
            if($.trim(code.val())==""){
                ...
                if(mobile!=""||code!=""){
                    ...
                    user.sendAjax(url, q,
                        function(json){
                            if(json.status == 0){$.showSuccess("对不起，验证码错误");user.formData.verifyCode.removeClass("correct");return false;}
        ...
        sendAjax: function (url, param, successFun, errorFun) {
            $.ajax({
                url: url,
                data: param,
                type: "POST",
