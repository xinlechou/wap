...
/*根据id获取分类标签名称 2015-12-01*/
function get_cate($id){
    $cate_list = $GLOBALS['db']->getAll("select name from ".DB_PREFIX."deal_cate where id = ".$id);
...
function get_pages($count){
    ...
    for($i=1;$i<=$count;$i++){
        if($i==$p){
            $page .= '<option selected="selected" value="'.$i.'">'.$i.'</option>';
        }else{
...
function getCateList(){
    $cates =$GLOBALS['db']->getAll("select *from ".DB_PREFIX."deal_cate order by sort asc");
    return $cates;
}
/*获取是支付方式-new-wap*/
function get_payment_list_wap($from="wap"){
    if($from=='wap'){
        $payment_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."payment where is_effect = 1 and online_pay=2 order by sort asc ");
        return $payment_list;
    }elseif($from=='ap'){
        $payment_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."payment where id = 41");
        return $payment_list;
    }
}
?>