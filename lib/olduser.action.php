...
//		$GLOBALS['tmpl']->display("user_login.html",$cache_id);
	...
	public function do_login()
	{
		...
		if(check_ipop_limit(get_client_ip(),"user_dologin",5)){
            ...
            $this->showResult("提交太快",$ajax,url_wap("olduser#login"));
        }
		if($result['status']){
			...
			if($ajax==0&&trim(app_conf("INTEGRATE_CODE"))=='') {
				$redirect = $_SERVER['HTTP_REFERER']?$_SERVER['HTTP_REFERER']:url_wap("old");
				app_redirect($redirect);
			}else{
				$jump_url = get_gopreview_old();
				if($ajax==1)
				{
					$return['status'] = 1;
					$return['info'] = "登录成功";
				...
				else
				{
					...
					$this->showResult("登录成功",$ajax,$jump_url);
				}
			}
		}
		else
	...
	public function register()
	{
		...
		$GLOBALS['tmpl']->display("old/olduser_register.html");
	}
	...
	public function loginout(){
		...
		if($result['status']){
			...
			else
			{
				...
				{
					app_redirect(get_gopreview_old());
				}
				else
				$this->showResult("登出成功",0,get_gopreview_old());
			}
		}
		else
		{
...
//		$cache_id  = md5(MODULE_NAME.ACTION_NAME);
		...
		$GLOBALS['tmpl']->display("old/olduser_getpassword.html");
	}
	...
	public function wx_register(){
		...
		$GLOBALS['tmpl']->assign('referer',$referer);
		$GLOBALS['tmpl']->display("old/olduser_wx_register.html");
	}
...
//			send_register_success(0,$user_info);
			...
			$this->showResult("验证成功",0,get_gopreview_old());
		}

		elseif($user_info['verify']=='')
		{
			$this->showResult("已验证过",0,get_gopreview_old());

		}
		else
		{
			$this->showResult("验证失败",0,get_gopreview_old());
		}
	}
        ...
        if(!$wx_info['unionid']){
            ...
            if($userinfo){
                ...
                if($unionid_idx&&$unionid_idx!==$userinfo['id']){
                    $this->showResult('此微信已与其它手机号绑定，请登录后至个人中心解绑，再绑定此微信号');
                    exit;
                }
                $userid = $userinfo['id'];
                //查询该用户是否有绑定信息
...
//                        echo $user_unionid;exit;
                        ...
                        $this->showResult('此账号已与其它微信绑定，请更换手机号，或用手机号登录');
                        exit;
                    }
                }else{
...
//                        print_r($sql_user_update);
        ...
        $html = "old/olduser_bind_mobile.html";
    ...
    public function do_bind_mobile(){
        ...
        //没有微信帐号信息
        if(!$user_info){$this->showResult("请使用微信扫描二维码后完成帐号绑定",1,"");}
        //判断微信号是否已存在
        $select_openid_sql = "select id from ".DB_PREFIX."user_idx where wechat_unionid = '".$user_info['unionid']."'";
        $unionid_idx = $GLOBALS['db']->getOne($select_openid_sql);
        if($unionid_idx){$this->showResult("此微信已与其它手机号绑定，请登录后至个人中心解绑，再绑定此微信号",1,"");}
...
//                    ajax_return(array("status"=>1,"info"=>$result['msg'],"jump"=>$referer));
        ...
        if($result['status']){
            ...
            $return['info'] = "登录成功";
            $return['data'] = $result['msg'];
            $return['jump'] = get_gopreview_old();//与pc端不同
            ajax_return($return);
        }else{
            switch($result['data']){
    ...
    public function wx_login(){
        ...
        $GLOBALS['tmpl']->display("old/olduser_wx_login.html");
    }
    /*增加old的成功失败页面*/
