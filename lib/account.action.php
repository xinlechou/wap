...
//		$p  =  $page->show();
        $page = get_pages(ceil($order_count/$page_size));
		...
		if($deal_ids!=null){
			...
			foreach($order_list as $k=>$v)
			{
                ...
                $order_list[$k]['cate'] = get_cate($deal_list[$v['deal_id']]['cate_id']);
                $order_list[$k]['deal_item_info'] = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_item where  id=".$order_list[$k]['deal_item_id']);
                if( $order_list[$k]['is_ap']){
                    $order_list[$k]['total_price'] *= $order_list[$k]['ap_ratio'];
                }
			}
            $GLOBALS['tmpl']->assign('order_count',$order_count);
...
//                            $GLOBALS['tmpl']->assign('msg','订单为空');
	...
	public function project()
	{
        ...
        {
            $remain_days = ceil(($v['end_time'] - NOW_TIME)/(24*3600));
            $deal_list[$k]['remain_days'] = $remain_days>0?$remain_days:"已过期";
