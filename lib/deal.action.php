...
//        print_r($dealImageArray);
		...
		if($deal_info['type']==1){
			...
			for($i=0;$i<count($deal_info['deal_item_list']);$i++){
                if($i==0){$deal_item_bottom_price = $deal_info['deal_item_list'][$i]['price'];}
                if($i==count($deal_info['deal_item_list'])-1){$deal_item_top_price = $deal_info['deal_item_list'][$i]['price'];}
//                $deal_info['deal_item_list'][$i]['ap_price_format'] = $deal_info['deal_item_list'][$i]['price_format']*$ratio;
            }
//            print_r($deal_info);
            if($is_ap == 1){
                $deal_item_bottom_price = $deal_item_bottom_price*$ratio;//用于临时展示，字段齐全后改为从字段取
...
//                es_session::start();
	...
	public function init_deal_page($deal_info)
	{
		...
		foreach ($deal_item_list as $k=>$v){
            ...
            //增加积分数量
            $deal_info['deal_item_list'][$k]['ap_price_format'] = number_price_format((int)$deal_info['deal_item_list'][$k]['price']*(int)$deal_info['deal_item_list'][$k]['ap_ratio']);
		}
